...
from fairseq.modules import AdaptiveBN
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        if self.encoder_attn is not None:
            residual = x
            x = self.maybe_batch_norm(self.encoder_attn_batch_norm, x, before=True)
            ...
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = residual + x
            x = self.maybe_batch_norm(self.encoder_attn_batch_norm, x, after=True)
